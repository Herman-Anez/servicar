...
## Summary
- 790 nodes · 1692 edges · 45 communities (37 shown, 8 thin omitted)
...
1. `Ticket` - 81 edges
2. `Empleado` - 55 edges
3. `TicketEstado` - 37 edges
4. `MockStore` - 35 edges
5. `HistorialEntry` - 30 edges
6. `PbStore` - 29 edges
...
Cohesion: 0.09
Nodes (19): CambiarEstadoDTO, CrearTicketDTO, EditarTicketDTO, TicketCamposEditables, store, ICambiarEstadoUseCase, ICrearTicketUseCase, IEditarTicketUseCase (+11 more)

### Community 1 - "Casos de Uso y Reglas de Negocio de Tickets"
Cohesion: 0.09
Nodes (26): FichasPage(), FichasView(), TABS, TallerView(), FichasVM, FichaTab, useFichasViewModel(), FilterEstado (+18 more)

### Community 2 - "Configuración de Compilación TypeScript (Next.js)"
Cohesion: 0.08
Nodes (18): provider, store, MOCK_EMPLEADOS, MOCK_HISTORIAL, MOCK_TICKETS, MockEmpleado, MockHistorial, MockTicket (+10 more)

### Community 3 - "Dominio de Empleados y Control de Acceso"
Cohesion: 0.11
Nodes (10): Empleado, EmpleadoProps, mockEmpleadoToEntity(), MockEmpleadoRepository, PbEmpleadoRepository, IEmpleadoRepository, IGetEmpleadoByIdQuery, IGetEmpleadosQuery (+2 more)
...
Cohesion: 0.10
Nodes (11): ESTADO_CFG, HistorialView(), HistorialTab, HistorialVM, useHistorialViewModel(), AuditTimeline(), AuditTimelineProps, InfoRow() (+3 more)
...
Cohesion: 0.20
Nodes (10): ColaView(), Action, ColaVM, PendingAction, useColaViewModel(), ColaPage(), ConfirmModal(), ConfirmModalProps (+2 more)
...
Cohesion: 0.12
Nodes (12): provider, useCase, Rol, AutenticarDTO, Sesion, SesionProps, authModule, PbAuthProvider (+4 more)
...
Cohesion: 0.06
Nodes (31): AggregateRoot, isTransicionValida(), TICKET_ESTADO_TRANSITIONS, TicketEstado, HistorialEntryProps, TipoAccion, TicketProps, MockHistorialRepository (+23 more)
...
Cohesion: 0.05
Nodes (41): AdminLayoutView(), AdminLayoutViewProps, NAV, AdminLayout(), AdminLayoutVM, useAdminLayoutViewModel(), useTicketsViewModel(), AdminCoordinator (+33 more)

### Community 48 - "Community 48"
Cohesion: 0.18
Nodes (7): ESTADO_ACTIONS, TicketsView(), TicketsVM, ESTADO_ACTIONS, KanbanCardProps, KanbanColumn(), KanbanColumnProps

## Knowledge Gaps
- **198 isolated node(s):** `name`, `private`, `workspaces`, `$schema`, `enabled` (+193 more)
...
- **Why does `Ticket` connect `Entidad de Dominio y Modelo de Ticket` to `Interfaz y Formularios de Tickets`, `Casos de Uso y Reglas de Negocio de Tickets`, `Community 37`, `Capa de Persistencia Simulada (Mock)`, `Gestión de Dependencias (package.json)`, `Community 42`, `Capa de Acceso a Datos de PocketBase`, `Community 48`, `Glosario y Reglas de Negocio (CLAUDE.md/README.md)`, `Repositorio de Tickets de PocketBase`, `Pruebas Unitarias de la Entidad Ticket`, `Arquitectura de Despliegue y Orquestación Docker`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `Empleado` connect `Dominio de Empleados y Control de Acceso` to `Casos de Uso y Reglas de Negocio de Tickets`, `Configuración de Compilación TypeScript (Next.js)`, `Community 37`, `Gestión de Dependencias (package.json)`, `Reglas de Calidad y Estilo de Código (Biome)`, `Community 42`, `Capa de Acceso a Datos de PocketBase`, `Community 48`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `PbStore` connect `Community 37` to `Interfaz y Formularios de Tickets`, `Dominio de Empleados y Control de Acceso`, `Community 42`, `Estructura y Maquetación Principal (Layout)`, `Panel de Administración y Control de Cola`, `Arquitectura de Despliegue y Orquestación Docker`?**
